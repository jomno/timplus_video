...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
            ...
            '<button class="ssi-button success" id="file">' + this.language.chooseFiles + '</button>' +
            '</span>').append(element);
        ...
        var $clearBtn = $('<button id="ssi-clearBtn" class="ssi-hidden ssi-button error" >' + this.language.clear +
            '</button>');
        ...
        this.$element.append($('<div class="ssi-buttonWrapper">').append($chooseBtn, $abortBtn, $clearBtn));
        var $uploadBox;
        ...
        //----------------------------UPLOADFILES------------------------------------
        $('#ssi-uploadBtn').click(function (e) {// upload the files
            if($('#ssi-uploadBtn').attr('class').split(' ').includes('add')){
                e.preventDefault();
                if($('#ssi-uploadBtn').attr('class')=="curriculum add"){
                    var data={"curriculum[authenticity_token]":$('input').eq(1).attr("value"),"curriculum[title]": $('#curriculum_title').val(),"curriculum[description]":$('#curriculum_description').val(),"video_upload[category_id]":$('#video_upload_category_id').val()}
                }else if($('#ssi-uploadBtn').attr('class')=="article add"){
                    var data={"article[authenticity_token]":$('input').eq(1).attr("value"),"article[title]": $('#article_title').val(),"article[videolink]": $('#article_videolink').val(),"article[description]":$('#article_description').val()}
                }
        ...
        $('#file').click(function(e){
            console.log('hi');
            $('.curriculum').attr('class','curriculum add');
            $('.article').attr('class','article add');
        });
    ...
    function add_remove_id(status){
        if(status=="add"){
            console.log('add');
            $('.curriculum').attr('id','ssi-uploadBtn');
            $('.article').attr('id','ssi-uploadBtn');
        }else{
            console.log('remove');
            $('.curriculum').removeAttr('id');
